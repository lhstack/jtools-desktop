...
- 调整全局热键行为：`Alt+Space` 改为仅激活/前置启动器，不再切换隐藏状态。
- 修复启动器唤起后首个按键偶发失效问题（含插件新窗口场景下的焦点恢复）。
- 统一启动器与管理中心宽度，减少切换时视觉跳变。
- 修复打包后内置插件丢失问题：将 `plugins/builtin` 纳入打包资源，并增强运行时内置插件路径探测。
- 新增本地 Windows 打包预处理脚本：自动准备 `WebView2Loader.dll`，避免本地 `tauri:build:win` 构建失败。
...
4. 发布构建会按架构准备 `src-tauri/resources/WebView2Loader.dll`，并通过 `bundle.resources + installer-hooks.nsh` 复制到安装目录。
